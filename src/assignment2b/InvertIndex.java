...
public class InvertIndex extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
	    ...
	    FileInputFormat.addInputPath(idjob, new Path("ex2/part-r-00000"));
		FileOutputFormat.setOutputPath(idjob, new Path("finalOutput"));
	...
	private static class Map extends Mapper<LongWritable, Text, Text, Text>{
	    ...
	    public void map(LongWritable offset, Text lineText, Context context)
	      ...
	      int words = context.getConfiguration().getInt("words", 0);
	      if (!caseSensitive) {
	      ...
	      context.getConfiguration().setInt("words", words);
	    }
		...
		protected void cleanup(
			...
			FSDataOutputStream outStream = fs.create(path);
	  		outStream.writeInt(context.getConfiguration().getInt("words", -1));
		 ...
		 @Override
		    public void reduce(Text word, Iterable<Text> docs, Context context)
		      ...
		      for (Text doc : docs) {
		    	  ...
		    	  if(previous.equals(doc.toString()) || doc.toString().isEmpty() || doc.getLength() == 0) continue;
